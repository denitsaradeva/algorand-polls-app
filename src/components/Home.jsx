...
const Home = () => {
  ...
  return (
    <div className="justify-content-center d-flex flex-column poll-footer text-center min-vh-100">
      <div>
        <div style={{ maxWidth: "320px" }}></div>
        <h1 className="text-light display-1">{"Algo Polls"}</h1>
        <Button
          type="button"
          onClick={() => connectToMyAlgoWallet()}
          className="btn-light rounded-pill mt-4"
        >
          Connect Wallet
        </Button>
        {isWaiting && (
          <div className="flex waitingSpinner justify-content-center">
        ...
        {address && (
          <Link to="/polls" state={pollsData}>
            <Button type="button" className="btn-light rounded-pill mt-4">
              Go to Polls
            </Button>
          </Link>
        )}
